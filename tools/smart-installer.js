...
class APPIQSmartInstaller {
  ...
  async setupIDECommands() {
    console.log('🔧 Setting up IDE commands...');
    ...
    try {
      ...
      // Create .cursor directory and commands
      ...
      // Create .claude directory and commands  
      ...
      for (const [name, config] of Object.entries(commands)) {
        const commandContent = this.generateCommandFile(name, config);
        
        // Write to all command directories
        fs.writeFileSync(path.join(commandsDir, `${name}.md`), commandContent);
        fs.writeFileSync(path.join(cursorCommandsDir, `${name}.md`), commandContent);
        fs.writeFileSync(path.join(claudeCommandsDir, `${name}.md`), commandContent);
      }
      ...
      console.log('✅ IDE commands configured');
      console.log('   📁 ./commands/ - Generic command files');
      console.log('   📁 ./.cursor/commands/ - Cursor IDE commands');  
      console.log('   📁 ./.claude/commands/ - Claude IDE commands');
      
    } catch (error) {
      console.log('⚠️  Failed to setup IDE commands:', error.message);
      console.log('   You can manually run: node tools/setup-ide-commands.js');
    }
  }
...
## Usage
Type \`/${name}\` in your IDE to activate the APPIQ Smart Launcher.

## What happens:
1. **Context Analysis** - Analyzes your current project setup
2. **Agent Activation** - Loads the ${config.agent} agent
...
`;
  ...
  async completionGuide() {
    ...
    console.log('   1. Activate APPIQ in your IDE:');
    console.log('      node activate-appiq.js');
    console.log('   2. Copy the Smart Launcher Agent to your IDE');
    console.log('   3. Type /appiq to start your first project');
    console.log('   4. Follow the intelligent guided workflow');
    console.log('');
